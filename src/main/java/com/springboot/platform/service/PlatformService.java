...
//        String pathWithoutExt = "platform/" + platform.getPlatformName();
//        String imageUrl = storageService.store(image, pathWithoutExt);
        String fileName = image.getOriginalFilename();

        String pathWithoutExt = "platform/" + fileName;
    ...
    //이미지 등록
    public void uploadImage(Platform platform, MultipartFile multipartFile){
        Platform findPlatform = findVerifiedPlatform(platform.getPlatformId());

        if (multipartFile != null && !multipartFile.isEmpty()){
            // 프로필 이미지 덮어쓰기 되도록 구현
            String pathWithoutExt = "platform/" + findPlatform.getPlatformName();
            String imageUrl = storageService.store(multipartFile, pathWithoutExt);
            findPlatform.setPlatformImage(imageUrl);
        }
    }
}
